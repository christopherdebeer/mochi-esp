...
/* NOT VENDOR CODE — adopt-handle path. Builds an empty wrapper and
 * stamps in an externally-owned bus handle, skipping the vendor
 * constructor's `i2c_new_master_bus`. Used when another component
 * (codec_board, in our project) is the canonical bus owner. The
 * timing tick globals at the top of the file are still set so
 * the i2c_*_buff helpers behave the same. */
I2cMasterBus *I2cMasterBus::adoptHandle(i2c_master_bus_handle_t external) {
    if (instance_ == NULL) {
        instance_ = new I2cMasterBus();
    }
    instance_->user_i2c_handle = external;
