...
public:
    ...
    /*
     * NOT VENDOR CODE — added 2026-05-18 for the eink-pet M4
     * milestone. Copies an external 1-bit packed framebuffer (same
     * layout the SSD1681 RAM expects: byte_index = y*25 + (x>>3),
     * MSB-first within each byte; 0=ink, 1=paper) into the
     * driver's internal `buffer`. Subsequent EPD_Display() /
     * EPD_DisplayPart() then renders it. Calls with len > buffer
     * size are clamped; calls with len < buffer size leave the
