...
/* NOT VENDOR CODE — added 2026-05-18 for eink-pet M4. See header. */
void epaper_driver_display::EPD_LoadBuffer(const uint8_t *src, size_t len) {
    assert(buffer);
    if (!src) return;
    size_t cap = (size_t)lcd_spi_data.buffer_len;
    if (len > cap) len = cap;
